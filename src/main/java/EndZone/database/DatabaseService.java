...
public class DatabaseService {
    ...
    private static void migrateZreTablesToEz() {
        String[] zreTables = {
            "zre_active_mutes", "zre_channel_restrictions", "zre_general_logs",
            "zre_message_logs", "zre_moderation_analytics", "zre_mute_config",
            "zre_user_voice_stats", "zre_voice_activity", "zre_voice_channels",
            "zre_voice_server_setup"
        };

        try (Connection conn = dataSource.getConnection()) {
            boolean anyZreTable = false;
            DatabaseMetaData meta = conn.getMetaData();
            for (String table : zreTables) {
            ...
            System.out.println("[DATABASE] Migrating legacy zre_* tables into ez_* equivalents...");
            try (Statement stmt = conn.createStatement()) {
                stmt.execute("""
                    INSERT OR IGNORE INTO ez_active_mutes (guildId, userId, unmuteTime)
                    SELECT guildId, userId, unmuteTime FROM zre_active_mutes
                    """);
                stmt.execute("""
                    UPDATE ez_active_mutes
                    SET unmuteTime = CASE
                      WHEN ez_active_mutes.unmuteTime = 0 OR z.unmuteTime = 0 THEN 0
                      WHEN z.unmuteTime > ez_active_mutes.unmuteTime THEN z.unmuteTime
                      ELSE ez_active_mutes.unmuteTime
                    END
                    FROM zre_active_mutes z
                    WHERE ez_active_mutes.guildId = z.guildId AND ez_active_mutes.userId = z.userId
                    """);

                stmt.execute("""
                    INSERT OR IGNORE INTO ez_channel_restrictions (channelId, restrictionType)
                    SELECT channelId, restrictionType FROM zre_channel_restrictions
                    """);

                stmt.execute("""
                    INSERT INTO ez_general_logs (guildId, userId, eventType, details, timestamp)
                    SELECT guildId, userId, eventType, details, timestamp FROM zre_general_logs
                    """);

                stmt.execute("""
                    INSERT INTO ez_message_logs (guildId, channelId, messageId, userId, content, action, timestamp)
                    SELECT guildId, channelId, messageId, userId, content, action, timestamp FROM zre_message_logs
                    """);

                stmt.execute("""
                    INSERT INTO ez_moderation_analytics
                      (action, moderatorId, moderatorName, targetId, targetName, reason, timestamp, duration, count, created_at, guildName, channelName)
                    SELECT action, moderatorId, moderatorName, targetId, targetName, reason, timestamp, duration, count, created_at,
                           'Unknown Server', 'Unknown Channel'
                    FROM zre_moderation_analytics
                    """);

                stmt.execute("""
                    INSERT OR IGNORE INTO ez_mute_config (guildId, muteRoleId)
                    SELECT guildId, muteRoleId FROM zre_mute_config
                    """);

                stmt.execute("""
                    UPDATE ez_user_voice_stats
                    SET
                      user_name = z.user_name,
                      total_channels_created = ez_user_voice_stats.total_channels_created + z.total_channels_created,
                      total_voice_time_seconds = ez_user_voice_stats.total_voice_time_seconds + z.total_voice_time_seconds,
                      last_channel_created = CASE
                        WHEN ez_user_voice_stats.last_channel_created IS NULL THEN z.last_channel_created
                        WHEN z.last_channel_created IS NULL THEN ez_user_voice_stats.last_channel_created
                        WHEN z.last_channel_created > ez_user_voice_stats.last_channel_created THEN z.last_channel_created
                        ELSE ez_user_voice_stats.last_channel_created
                      END,
                      last_voice_activity = CASE
                        WHEN ez_user_voice_stats.last_voice_activity IS NULL THEN z.last_voice_activity
                        WHEN z.last_voice_activity IS NULL THEN ez_user_voice_stats.last_voice_activity
                        WHEN z.last_voice_activity > ez_user_voice_stats.last_voice_activity THEN z.last_voice_activity
                        ELSE ez_user_voice_stats.last_voice_activity
                      END,
                      updated_at = CASE
                        WHEN ez_user_voice_stats.updated_at IS NULL THEN z.updated_at
                        WHEN z.updated_at IS NULL THEN ez_user_voice_stats.updated_at
                        WHEN z.updated_at > ez_user_voice_stats.updated_at THEN z.updated_at
                        ELSE ez_user_voice_stats.updated_at
                      END
                    FROM zre_user_voice_stats z
                    WHERE ez_user_voice_stats.user_id = z.user_id AND ez_user_voice_stats.guild_id = z.guild_id
                    """);
                stmt.execute("""
                    INSERT INTO ez_user_voice_stats
                      (user_id, user_name, guild_id, total_channels_created, total_voice_time_seconds,
                       last_channel_created, last_voice_activity, created_at, updated_at)
                    SELECT z.user_id, z.user_name, z.guild_id, z.total_channels_created, z.total_voice_time_seconds,
                           z.last_channel_created, z.last_voice_activity, z.created_at, z.updated_at
                    FROM zre_user_voice_stats z
                    WHERE NOT EXISTS (
                      SELECT 1 FROM ez_user_voice_stats e
                      WHERE e.user_id = z.user_id AND e.guild_id = z.guild_id
                    )
                    """);

                stmt.execute("""
                    INSERT INTO ez_voice_activity
                      (channel_id, user_id, user_name, guild_id, action_type, joined_at, left_at, duration_seconds, created_at)
                    SELECT channel_id, user_id, user_name, guild_id, action_type, joined_at, left_at, duration_seconds, created_at
                    FROM zre_voice_activity
                    """);

                stmt.execute("""
                    INSERT INTO ez_voice_channels
                      (channel_id, channel_name, creator_id, creator_name, guild_id, guild_name, category_id,
                       user_limit, channel_type, created_at, deleted_at, is_active)
                    SELECT channel_id, channel_name, creator_id, creator_name, guild_id, guild_name, category_id,
                           user_limit, channel_type, created_at, deleted_at, is_active
                    FROM zre_voice_channels
                    """);

                stmt.execute("""
                    INSERT OR IGNORE INTO ez_voice_server_setup (guild_id, category_id, managed_vc_ids, created_at, updated_at)
                    SELECT guild_id, category_id, managed_vc_ids, created_at, updated_at FROM zre_voice_server_setup
                    """);
            ...
            System.out.println("[DATABASE] Legacy zre_* tables merged and removed");
        } catch (SQLException e) {
            System.err.println("[ERROR] Failed to migrate zre_* tables: " + e.getMessage());
            e.printStackTrace();
        }
    }

    private static void dropLegacyTables() {
        String[] legacyTables = {"blacklist", "restricted_users", "ez_warnings"};
        try (Connection conn = dataSource.getConnection();
            ...
            System.err.println("[ERROR] Failed to drop legacy tables: " + e.getMessage());
        }
    }
