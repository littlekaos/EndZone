...
public class DemotionService {
        ...
        if (actionRow != null) {
            channel.editMessageEmbedsById(messageId, embed)
                    ...
                    .queue(
                        ...
                        error -> {
                            System.err.println("DEBUG: Failed to update demotion list message: " + error.getMessage());
                            if (error.getMessage().contains("10008")) {
                                System.err.println("DEBUG: Message was deleted. Clearing ID from database.");
    ...
    public List<MessageEmbed> buildPages() {
        ...
        for (String userId : permDemotions) {
            permLines.add(String.format("<@%s> (%s) 3 strikes, permanently demoted", userId, userId));
        }
        ...
        for (String userId : allUsersWithStrikes) {
            ...
            if (strikeCount >= 3) {
                permLines.add(String.format("<@%s> (%s) %d strikes, permanently demoted", userId, userId, strikeCount));
            }
        }
