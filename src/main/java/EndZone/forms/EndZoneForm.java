...
import EndZone.util.FormErrorService;
...
public class EndZoneForm {
    ...
    public static void startApplication(User user, ButtonInteractionEvent event) {
        ...
        if (existingState != null && existingState.currentQuestion > 1 && !existingState.answers.isEmpty()) {
            ...
            user.openPrivateChannel().queue(
                ...
                error -> FormErrorService.sendErrorNotification("Failed to open DM channel for user " + user.getAsTag() + ": " + error.getMessage())
            );
        } else {
            ...
            user.openPrivateChannel().queue(
                privateChannel -> {
                    ...
                    privateChannel.sendMessageEmbeds(embed.build())
                            .addComponents(ActionRow.of(Button.primary("dm_ready_button", "Ready to Start")))
                            .queue(
                                null,
                                error -> FormErrorService.sendErrorNotification("Failed to send application welcome message to " + user.getAsTag() + ": " + error.getMessage())
                            );
                },
                error -> FormErrorService.sendErrorNotification("Failed to open DM channel for user " + user.getAsTag() + ": " + error.getMessage())
            );
        ...
        switch (questionNum) {
            case 1:
                modal = Modal.create("q1_modal", "Question 1")
                        .addComponents(Label.of("What is your Discord Username & ID#?", TextInput.create("q1_answer", TextInputStyle.SHORT)
            ...
            case 6:
                modal = Modal.create("q6_modal", "Question 6")
                        .addComponents(Label.of("Host/Streamer Apps", TextInput.create("q6_answer", TextInputStyle.SHORT)
            ...
            case 9:
                modal = Modal.create("q9_modal", "Question 9")
                        .addComponents(Label.of("Tell us a bit about yourself", TextInput.create("q9_answer", TextInputStyle.PARAGRAPH)
            ...
            case 10:
                modal = Modal.create("q10_modal", "Question 10")
                        .addComponents(Label.of("Why should we offer you staff?", TextInput.create("q10_answer", TextInputStyle.PARAGRAPH)
            ...
            case 11:
                modal = Modal.create("q11_modal", "Question 11")
                        .addComponents(Label.of("What would you add to Endzone?", TextInput.create("q11_answer", TextInputStyle.PARAGRAPH)
            ...
            case 13:
                modal = Modal.create("q13_modal", "Question 13")
                        .addComponents(Label.of("Please let us know your experience.", TextInput.create("q13_answer", TextInputStyle.PARAGRAPH)
            ...
            case 14:
                modal = Modal.create("q14_modal", "Question 14")
                        .addComponents(Label.of("How would you help the event if given staff?", TextInput.create("q14_answer", TextInputStyle.PARAGRAPH)
            ...
            case 15:
                modal = Modal.create("q15_modal", "Question 15")
                        .addComponents(Label.of("You've been given an elephant...", TextInput.create("q15_answer", TextInputStyle.PARAGRAPH)
    ...
    private static void presentQuestionDM(FormState state, User user) {
        ...
        user.openPrivateChannel().queue(
            ...
            error -> FormErrorService.sendErrorNotification("Failed to open DM channel for question " + questionNum + " for user " + user.getAsTag() + ": " + error.getMessage())
        );
    }
    ...
    private static void presentDiscordUsernameQuestionDM(FormState state, net.dv8tion.jda.api.entities.channel.concrete.PrivateChannel channel) {
        if (channel != null) {
            channel.sendMessage("**Question 1:** What is your Discord Username & ID#?")
                    .addComponents(ActionRow.of(Button.primary("q1_answer_button", "Answer Question 1")))
                    .queue(
                        null,
    ...
    private static void presentRoleSelectionQuestionDM(FormState state, net.dv8tion.jda.api.entities.channel.concrete.PrivateChannel channel) {
        ...
        channel.sendMessageEmbeds(embed.build())
                ...
                .queue(
                    null,
                    error -> FormErrorService.sendErrorNotification("Failed to send Question 2 to " + channel.getUser().getAsTag() + ": " + error.getMessage())
                );
    }
    ...
    private static void presentWeekdayModerationQuestionDM(FormState state, net.dv8tion.jda.api.entities.channel.concrete.PrivateChannel channel) {
        ...
        channel.sendMessageEmbeds(embed.build())
                .addComponents(ActionRow.of(yesBtn, noBtn))
                .queue(
                    null,
    ...
    private static void presentTwitchLinkQuestionDM(FormState state, net.dv8tion.jda.api.entities.channel.concrete.PrivateChannel channel) {
        if (channel != null) {
            channel.sendMessage("**Question 6:** Host/Streamer Apps - Please provide your Twitch link.")
                    .addComponents(ActionRow.of(Button.primary("q6_answer_button", "Answer Question 6")))
                    .queue(
                        null,
                        error -> FormErrorService.sendErrorNotification("Failed to send Question 6 to " + channel.getUser().getAsTag() + ": " + error.getMessage())
                    );
        }
    }
    ...
    private static void presentAboutYourselfQuestionDM(FormState state, net.dv8tion.jda.api.entities.channel.concrete.PrivateChannel channel) {
        if (channel != null) {
            channel.sendMessage("**Question 9:** Tell us a bit about yourself.")
                    .addComponents(ActionRow.of(Button.primary("q9_answer_button", "Answer Question 9")))
                    .queue(
                        null,
    ...
    private static void presentWhyStaffQuestionDM(FormState state, net.dv8tion.jda.api.entities.channel.concrete.PrivateChannel channel) {
        if (channel != null) {
            channel.sendMessage("**Question 10:** Why should we offer you staff?")
                    .addComponents(ActionRow.of(Button.primary("q10_answer_button", "Answer Question 10")))
                    .queue(
                        null,
    ...
    private static void presentAddToEndzoneQuestionDM(FormState state, net.dv8tion.jda.api.entities.channel.concrete.PrivateChannel channel) {
        if (channel != null) {
            channel.sendMessage("**Question 11:** If you could add something in the Endzone, what would it be and why?")
                    .addComponents(ActionRow.of(Button.primary("q11_answer_button", "Answer Question 11")))
                    .queue(
                        null,
    ...
    private static void presentExperienceDetailsQuestionDM(FormState state, net.dv8tion.jda.api.entities.channel.concrete.PrivateChannel channel) {
        if (channel != null) {
            channel.sendMessage("**Question 13:** If you answered Yes above, please let us know your experience.")
                    .addComponents(ActionRow.of(Button.primary("q13_answer_button", "Answer Question 13")))
                    .queue(
                        null,
                        error -> FormErrorService.sendErrorNotification("Failed to send Question 13 to " + channel.getUser().getAsTag() + ": " + error.getMessage())
                    );
        }
    }
    ...
    private static void presentContributionQuestionDM(FormState state, net.dv8tion.jda.api.entities.channel.concrete.PrivateChannel channel) {
        if (channel != null) {
            channel.sendMessage("**Question 14:** If you are given staff, what and how would you contribute towards the event?")
                    .addComponents(ActionRow.of(Button.primary("q14_answer_button", "Answer Question 14")))
                    .queue(
                        null,
    ...
    private static void presentElephantQuestionDM(FormState state, net.dv8tion.jda.api.entities.channel.concrete.PrivateChannel channel) {
        if (channel != null) {
            channel.sendMessage("**Question 15:** You've been given an elephant. You can't give it away or sell it. What would you do with the elephant? (This is a Brulph Original, so it will always stay... just answer as best you can. ;o)")
                    .addComponents(ActionRow.of(Button.primary("q15_answer_button", "Answer Question 15")))
                    .queue(
                        null,
                        error -> FormErrorService.sendErrorNotification("Failed to send Question 15 to " + channel.getUser().getAsTag() + ": " + error.getMessage())
                    );
        }
    }
    ...
    private static void submitApplicationDM(FormState state, User user) {
        ...
        user.openPrivateChannel().queue(
            privateChannel -> {
                ...
                privateChannel.sendMessageEmbeds(confirmEmbed.build()).queue(
                    null,
                    error -> FormErrorService.sendErrorNotification("Failed to send application confirmation to " + user.getAsTag() + ": " + error.getMessage())
                );
            },
            error -> FormErrorService.sendErrorNotification("Failed to open DM channel for user " + user.getAsTag() + " during submission: " + error.getMessage())
        );
        ...
        if (ServiceManager.getJda() != null) {
            ...
            if (staffAppsChannel != null) {
                ...
                for (int groupIndex = 0; groupIndex < groupRanges.length; groupIndex++) {
                    ...
                    if (finalGroupIndex == groupRanges.length - 1) {
                        ...
                        staffAppsChannel.sendMessageEmbeds(embed.build())
                                .addComponents(ActionRow.of(approveBtn, denyBtn))
                                .queue(
                                    null,
                                    error -> FormErrorService.sendErrorNotification("Failed to post submission part " + (finalGroupIndex + 1) + " to review channel for " + user.getAsTag() + ": " + error.getMessage())
                                );
                    } else {
                        staffAppsChannel.sendMessageEmbeds(embed.build())
                                .queue(
                ...
                FormErrorService.sendErrorNotification("EndZone channel not found!");
            }
        } else {
            FormErrorService.sendErrorNotification("JDA instance is null - unable to post submission!");
        }
