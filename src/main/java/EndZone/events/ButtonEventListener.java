...
import EndZone.commands.HelpCommand;
...
public class ButtonEventListener extends ListenerAdapter {
    ...
    public void onButtonInteraction(ButtonInteractionEvent event) {
        ...
        if (buttonId.startsWith("help_")) {
            handleHelpNavigation(event);
        } else if (buttonId.startsWith("demotion_")) {
    ...
    private void handleDemotionListNavigation(ButtonInteractionEvent event) {
        ...
        // Update stored message ID so automatic updates target the active message
        ServiceManager.getStrikeService().getDatabase().setDemotionListMessageId(event.getMessageId());

        event.getHook().editOriginalEmbeds(embed).setComponents(buttons).queue();
    }

    private void handleHelpNavigation(ButtonInteractionEvent event) {
        String buttonId = event.getComponentId();
        boolean isCourtZone = HelpCommand.isCourtZone(event.getGuild());
        ...
        switch (buttonId) {
            case "help_court_general" -> HelpCommand.addCourtZoneGeneral(embed);
            case "help_court_mod" -> HelpCommand.addCourtZoneMod(embed);
